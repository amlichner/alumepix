...
String.prototype.originalTrim = String.prototype.trim;
String.prototype.trim = function(charlist) {
  return charlist === undefined? this.originalTrim() : this.trimLeft(charlist).trimRight(charlist);
};
...
window.flitsApp.wishlist_products = function () {
  ...
  this.change_page = function (page_number) {
    ...
    /* **********  start  20-09-2019 04:26pm   by THOR from FLITS ********** */
    var old_parameters = location.search.replace('?','').split('&');
    var old_p_index = this.searchStringInArray('p=',old_parameters);
    if(old_p_index != -1){
      old_parameters.splice(old_p_index,1);
    }
    old_parameters.push('p='+page_number);
    var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + '?'+old_parameters.join('&') + location.hash;
    /* **********  end  20-09-2019 04:26pm   by THOR from FLITS ********** */
    var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + '?p=' + page_number + location.hash;
    window.history.pushState({path: newurl}, '', newurl);
  };
  this.searchStringInArray = function(str, strArray) {
    ...
    if (that.isNull(customer_email)){
      ...
      if (that.isNull(customer_email_cookie)) {
        ...
        that.setCookie('flits_product_data', window.btoa(JSON.stringify(wishlist_data)), 15);
        that.setCookie('flits_previous_url',location.href);
    ...
    item.classList.add("flits-disable-btn");
    ...
    switch (page) {
      ...
      case "collection-page":
        var add_wsl = document.querySelectorAll(".flits-add-to-wsl-btn[data-flits-product-id='" + product_id + "']");
        for (i = 0; i < add_wsl.length; i++) {
          add_wsl[i].style.display = "none";
        }

        var rmv_wsl = document.querySelectorAll(".flits-remove-from-wsl-btn[data-flits-product-id='" + product_id + "']");
        for (i = 0; i < rmv_wsl.length; i++) {
          rmv_wsl[i].style.display = "inline-flex";
        }

        var a = document.querySelectorAll(".flits-wsl-product-count[data-flits-product-id='" + product_id + "']"); 
        for (i = 0; i < a.length; i++) {
          count = parseInt(a[i].innerHTML);
          wsl_product_count = parseInt(count) + 1;
        }
        var c =  document.querySelectorAll(".flits-wsl-product-count[data-flits-product-id='" + product_id + "']");
        for (i = 0; i < c.length; i++) {
          c[i].innerHTML = wsl_product_count;
        }
        break;
    }
    ...
    ajax.onreadystatechange = function () {
      if (this.readyState == 4) {
        if (this.status == 200) {
          ...
          var disable_btn = document.querySelectorAll(".flits-disable-btn");
          for(var i=0;i<disable_btn.length;i++){
            disable_btn[i].classList.remove("flits-disable-btn");
          }
...
// added by Hulk on 24 Jan 2020
// code added for add product to wishlist => login page => where the product was(collection/product page)
// start
window.flitsApp.setWishlist = function(){
  var flits_product_data = window.flitsApp.wishlist_products_obj.getCookie('flits_product_data');
  if (flits_product_data != "") {
    flits_product_data = JSON.parse(window.atob(flits_product_data));
    var redirect_to = flits_product_data.redirect_to;
   debugger;
    if(document.querySelector("form#customer_login")){
      if(document.querySelector("form#customer_login input[name='checkout_url']")){
        document.querySelector("form#customer_login input[name='checkout_url']").value = redirect_to;
      }
      else{
        var flits_checkout_url_input = document.createElement('input');
        flits_checkout_url_input.type = "hidden";
        flits_checkout_url_input.name = "checkout_url";
        flits_checkout_url_input.value = redirect_to;
        document.querySelector("form#customer_login").appendChild(flits_checkout_url_input);
      }
    }

    var flits_customer_email = document.querySelector('#flits-customer-email').value;
    if (flits_customer_email != "") {
      flits_product_data.customer_email = flits_customer_email;

      //       var ajax = window.flitsApp.wishlist_products_obj.get_ajax_obj();
      //       ajax.onreadystatechange = function () {
      //         if (this.readyState == 4) {
      //           if (this.status == 200) {
      //             var res = JSON.parse(this.responseText);
      //             if(res.customer_wishlist_count > 0){
      //               window.flitsApp.flits_wishlist_color_update(true);
      //             }else{
      //               window.flitsApp.flits_wishlist_color_update(false);
      //             }
      //             window.flitsApp.wishlist_products_obj.setCookie('flits_products_handle', res.products_handle, 15);
      //             window.flitsApp.wishlist_products_obj.setCookie('flits_product_data', '', 0);
      //             window.flitsApp.wishlist_products_obj.setCookie('flits_scroll_data', flits_product_data.scroll_top, 1);
      //             if(location.href != redirect_to){
      ...
      //         }
      //       };
      //       ajax.open(flits_product_data.method, flits_product_data.url, true);
      //       ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
      //       var data = window.flitsApp.wishlist_products_obj.serialize(flits_product_data);
      //       ajax.send(data);

      if(location.href == redirect_to){
//         window.flitsApp.wishlist_products_obj.setCookie('flits_products_handle', res.products_handle, 15);
        window.flitsApp.wishlist_products_obj.setCookie('flits_product_data', '', 0);
        document.querySelector('.flits-add-to-wsl-btn[data-flits-product-id="'+flits_product_data.product_id+'"]').click();
        setTimeout(function(){ 
          window.scrollTo(0,flits_product_data.scroll_top); 
        }, 500);
      }
      else{
        location.href = redirect_to;
